...
import { radiologyDepartment } from "../../public/endpoins";
export default function CommitteeMemberDashboard() {
  ...
  const [tatActive, setTatActive] = useState("");
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.success) {
          ...
          setTatPending(res.data.tatPending);
          setTatActive(res.data.tatActive);
        }
      })
      .catch((error) => {
        console.log(error);
    ...
    <div className="container-fluid" style={{ marginBottom: 10 }}>
      <div className="row">
        ...
        >
          <div>
            <Dialer
              mainHeading={"Radiology Imaging Pending"}
              value={imagePending}
              color={imagePendingColor}
              subHeading={"TAT"}
              childHeading={"Request to Imaging"}
              time={tatPending ? tatPending : "00"}
            />
          </div>
        </div>
        <div
        ...
        >
          <div>
            <Dialer
              ...
              subHeading={"TAT"}
              childHeading={"Imaging to Results"}
              time={tatActive ? tatActive : "00"}
            />
          </div>
        </div>
      </div>

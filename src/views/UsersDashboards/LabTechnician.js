...
export default function CommitteeMemberDashboard() {
  ...
  const [tatImagePending, setTatImagePending] = useState("");
  const [tatResultPending, setTatResultPending] = useState("");
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.success) {
          ...
          setTatImagePending(res.data.tatPending);
          setTatResultPending(res.data.tatActive);
        }
      })
      .catch((error) => {
        console.log(error);
        ...
        >
          <div>
            <Dialer
              ...
              time={tatImagePending ? tatImagePending : "00"}
            />
          </div>
        </div>
        <div
        ...
        >
          <div>
            <Dialer
              ...
              time={tatResultPending ? tatResultPending : "00"}
            />
          </div>
        </div>
      </div>

...
export default function EDR(props) {
  ...
  function getEDRsData() {
    axios
      .get(getECRFromIPRUrl + '/' + currentUser.staffId)
      .then((res) => {
        if (res.data.success) {
          console.log(res.data.data, 'ipr')
          let temp = []
          for (let i = 0; i < res.data.data.length; i++) {
            ...
            let obj = {
              ...
              status: res.data.data[i].iprId.status,
            }
            temp.push(obj)
          }
          temp.map((d) => (d.patientId = d.iprId.patientId))
          console.log(temp)
          setEdr(temp.reverse())

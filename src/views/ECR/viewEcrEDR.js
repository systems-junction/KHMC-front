...
function AddEditPurchaseRequest(props) {
  ...
  useEffect(() => {
    if (props.history.location.state.tabValue) {
      setValue(2)
    }
    ...
    //     if (val && typeof val === "object") {
    //       if (key === "edrId") {
    //         dispatch({ field: "residentNoteArray", value: val.residentNotes });
    //         dispatch({
    //           field: "consultationNoteArray",
    //           value: val.consultationNote,
    //         });
    //         dispatch({ field: "labRequestArray", value: val.labRequest });
    //         dispatch({
    //           field: "radiologyRequestArray",
    //           value: val.radiologyRequest,
    //         });
    //       }
    //       if (key === "patientId") {
    //         dispatch({ field: "patientId", value: val._id });
  ...
  const handleChange = (event, newValue) => {
    console.log('THIS IS HANDLE CHANGE', newValue)
    setValue(newValue)
  }
      ...
      axios
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            ToastsStore.success('External consultation note  has been added')
          } else if (!res.data.success) {
            setOpenNotification(true)
  ...
  const addNewRequest = () => {
    ...
    props.history.push({
      ...
      state: {
        ...
        pharmacyRequestArray: pharmacyRequestArray,
      },
    })
  }
    ...
    axios
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          // window.location.reload(false)
          getEDRById(props.history.location.state.selectedItem._id)
          ToastsStore.success('Lab request has been added')
        } else if (!res.data.success) {
          setOpenNotification(true)
        }
      })
    ...
    axios
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          // window.location.reload(false)
          getEDRById(props.history.location.state.selectedItem._id)
          ToastsStore.success('Radiology request has been added')
        } else if (!res.data.success) {
          setOpenNotification(true)
        }
      })
        ...
        <div className='cPadding'>
          <div className='subheader'>
            <div>
              ...
              <h4>External Consultation Request</h4>
            </div>

...
import {
  getFuInventoryUrl,
  deleteBuInventoryUrl,
  getBuInventoryByIdUrl,
...
const tableHeading = ["BU Name", "Item Name", "Qty", "Actions"];
...
export default function BuInventory(props) {
  ...
  function getBuInventory() {
    axios
      .get(
        `${getBuInventoryByIdUrl}/${props.history.location.state.selectedItem._id}`
      )
      .then((res) => {
        if (res.data.success) {
  ...
  const addNewItem = () => {
    ...
    props.history.push({
      ...
      state: {
        ...
        items,
        businessUnit,
        buId: props.history.location.state.selectedItem,
      },
    });
  };

  function handleEdit(rec) {
    let path = `buinventory/edit`;
    props.history.push({
      pathname: path,
      state: {
        comingFor: "edit",
  ...
  function deleteBuInventory() {
    console.log("function called for delte");
